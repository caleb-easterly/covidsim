...
estimate_lm <- function(df,
                        covid_end_est = c(2020, 6)) {
  ...
  sim_times <- time(simd)
  covid_ind_est <- sim_times %in% 
    time(window(simd, start = covid_start, end = covid_end_est))
  
  ## indicator variable with on/off for COVID
  df$covid_ind_est <- covid_ind_est
